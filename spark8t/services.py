...
class K8sServiceAccountRegistry(AbstractServiceAccountRegistry):
    ...
    def _retrieve_account_configurations(
        ...
        user_properties = PropertyFile(
            {
                self._kubernetes_key_serializer.deserialize(key): value
                for key, value in secret.items()
            }
        )
        # get options generated by the configuration hub
        try:
            hub_secret = self.kube_interface.get_secret(
                configuration_hub_secret_name, namespace=namespace
            )["data"]
        except Exception:
            hub_secret = {}

        configuration_hub_properties = PropertyFile(
            {
                self._kubernetes_key_serializer.deserialize(key): value
                for key, value in hub_secret.items()
            }
        )

        return configuration_hub_properties + user_properties
